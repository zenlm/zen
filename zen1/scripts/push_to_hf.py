#!/usr/bin/env python3
"""
Publish Zen1 models to HuggingFace Hub
"""
...
def prepare_model_files():
    ...
    locations = {
        ...
        "zen1_instruct": Path("zen1/instruct"),
        "zen1_thinking": Path("zen1/thinking"),
    }
...
# Zen1{"-" + model_type.title() if model_type != "base" else ""}

{description} of Zen1, fine-tuned for advanced language understanding.
...
model = AutoModelForCausalLM.from_pretrained("zenlm/zen1{"-" + model_type if model_type != "base" else ""}")
tokenizer = AutoTokenizer.from_pretrained("zenlm/zen1{"-" + model_type if model_type != "base" else ""}")
...
```bash
ollama run zenlm/zen1{"-" + model_type if model_type != "base" else ""}
```
...
```bibtex
@misc{{zen1-2024,
  title={{Zen1{"-" + model_type.title() if model_type != "base" else ""}: Advanced Language Model}},
  author={{Zen Team}},
  year={{2024}},
  publisher={{HuggingFace}},
  url={{https://huggingface.co/zenlm/zen1{"-" + model_type if model_type != "base" else ""}}}
}}
```
"""
...
def upload_model(api, model_path, repo_id, model_type="base", token=None):
    ...
    try:
        ...
        api.upload_folder(
            ...
            repo_id=repo_id,
            repo_type="model",
            commit_message=f"Upload Zen1{'-' + model_type.title() if model_type != 'base' else ''} model"
        )
...
def main():
    ...
    if choice == str(len(options)+1):
        # Upload all
        uploads = [
            ("gym_model", "gym-output/model", "zenlm/zen1", "base"),
            ("zen1_instruct", "zen1/instruct", "zenlm/zen1-instruct", "instruct"),
            ("zen1_thinking", "zen1/thinking", "zenlm/zen1-thinking", "thinking"),
        ]
    else:
        # Upload selected
        idx = int(choice) - 1
        ...
        if selected == "gym_model":
            uploads = [("gym_model", available[selected], "zenlm/zen1", "base")]
        elif selected == "zen1_instruct":
            uploads = [("zen1_instruct", available[selected], "zenlm/zen1-instruct", "instruct")]
        elif selected == "zen1_thinking":
            uploads = [("zen1_thinking", available[selected], "zenlm/zen1-thinking", "thinking")]
